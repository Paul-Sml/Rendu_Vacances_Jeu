...
//-------------------------------------TOUR DU JOUEUR----------------------------------------------------------
int tourPlayer(int pvMonstre,int pvJoueur,int pmJoueur,int classe,int amuletteMagique){
	int choix;
	

	printf("Vous avez %d PV et %d PM, \nLe monstre a %d PV\n",pvJoueur,pmJoueur,pvMonstre );
	printf("Que voulez vous faire ? \nAttaque:1 Defense:2 Competence:3 Antidote:4\n");
	scanf("%d",&choix);
	if (choix == 1)
	{
		return choix;
	...
	if (choix == 3)
	{
		if ((classe == 1 && pmJoueur>=20) || (classe == 2 && pmJoueur>=20) || (classe==3 && pmJoueur>=15-amuletteMagique) || (classe==4 && pmJoueur>=10))
		{
			...
			tourPlayer(pvMonstre,pvJoueur,pmJoueur,classe,amuletteMagique);
		}
	}
	if (choix == 4)
	{
...
//------------------------------BOUCLE DE JEU-------------------------------------------------------------
	while(pvJoueur>0){
		...
		while(pvJoueur>0 && pvMonstre>0){
			...
			tour = tourPlayer(pvMonstre, pvJoueur, pmJoueur,classe,amuletteMagique);
			printf("%d\n",bleedL);
...
//----------------------------------EMPOISONNEMENT--------------------------------------------------------
			if (bleedL > 0) 
			{
				...
				printf("Vous etes empoisonne, vous perdez %d PV\n Il vous reste %d PV\n",bleedL, pvJoueur );
			}
//-------------------------------------ATTAQUE------------------------------------------------------------
			if (tour == 1)
			{
